...
function arrayCleanup() {
	...
	particles.map((particle) => {
		if (particle.size > 1.5) dump2.push(particle);
	});
	particles = dump2;
...
class Firework {
	constructor() {
		...
		this.y = h;
		this.targetY = getRandomInt(h * 0.15, h * 0.7);
		this.hue = hue;
		this.alpha = 1;
		this.tick = 0;
		this.ttl = getRandomInt(150, 200);
	}
	draw() {
...
class Particle {
	constructor(x, y, hue, i) {
		...
		this.speed = getRandomInt(30, 20) / 5;
	...
	draw() {
		if (this.size > 0) {
			...
			ctx.fillStyle = `hsla(${this.hue}, 25%, 25%, 1)`;
			ctx.fill();
			ctx.closePath();
		}
	}
...
class Circle {
	constructor(x, y, hue) {
		...
		this.hue = hue;
		this.size = 0;
		this.endSize = getRandomInt(70, 30);
	}
	draw() {
		if (this.size < this.endSize) {
			ctx.beginPath();
			ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2);
			ctx.fillStyle = `hsla(${this.hue}, 100%, 80%, .2)`;
			ctx.fill();
			ctx.closePath();
		}
	}
